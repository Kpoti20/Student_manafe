             ...
             if(isset($_POST['valider']))
              {
                        $ida=$_POST['ida'];
                        $st=$_POST['st'];
                        $_update = $bdd->prepare("UPDATE absence SET statut='$st'  WHERE ida=$ida ");
                        $_update->execute();
...
<?php 
      include_once 'index_control_presence_action.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <?php
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
            $idpo=$_GET['idpo'];
            $_select = $bdd->prepare("SELECT ida,etu,cour,statut FROM absence,etudiant WHERE absence.idetud=etudiant.idp and ida=$idpo ");
            $_select->execute();
...
<br/>
	<div class="w3-content">
            ...
            <form method="POST" action="">
             ...
             <p>
          <input type="hidden"  value="<?php echo $s->ida; ?>" readonly="true" name="ida">
          <label>Nom</label>
          <input class="w3-input" value="<?php echo $s->etu;?>" autocomplete="off" name="etu" type="text"></p>
          <p>
          <label>Statut</label>
            <select name="st" class="w3-select" id="st" >
                <option><?php echo $s->statut;?></option>
...
<?php 
  ...
  <div>
    ...
    Voulez-vous réellement supprimer l'étudiant(e) : <?php echo $z->etu; ?>
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->ida; ?>" readonly="true" name="code">
...
<?php
      ...
      <?php   
        ...
        ?>
  <p style="margin-left: 700px;"><b><i class="fa fa fa-arrow-left fa-fw w3-margin-right w3-text-teal"></i><a href="index_control_presence.php?idpo=<?php echo ($_SESSION['idpo']);?>" target="_parent" class="w3-padding-2" style="text-decoration:none;">Retour Controle de presence</a></b></p>
