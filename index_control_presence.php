   ...
   if(isset($_POST['forminscription']))
{
    ...
    for($i=1; $i<=$nbretud; $i++)
     { 
       if(isset($_POST['idetud'.$i]))
      {
          $idetud = $_POST['idetud'.$i];
          $st = $_POST['st'.$i];
          $datactu = date('Y-m-d');  
          $reqenre=$bdd->prepare("SELECT * FROM absence WHERE ljr ='$datactu'  and idetud = ? and cour = ? ");
           $reqenre->execute(array($idetud,$cour));
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                  $inserttut = $bdd->prepare("INSERT INTO absence(ljr,idetud,cour,statut) VALUES ('$datactu',?,?,?)");
                  $inserttut->execute(array($idetud,$cour,$st));
                  $upd = $bdd->prepare("DELETE FROM absence WHERE statut='Présent(e)'");
                  $upd->execute();
              } 
              else
              {
...
<?php 
      include_once 'index_control_presence.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <h2>CONTROLE DES PRESENCES AUX COURS PAR LES PROFESSEURS</h2>
</div>
<br/>
...
<?php   
    ...
    $jr=$_POST['jr'];
    $idprof= $_GET['idpo'];
    $_select = $bdd->prepare("SELECT idp,etu,mat FROM etudiant,seance WHERE jr='$jr' AND code_f='$fil' and idpro='$idprof'");
    $_select->execute();
...
?>    
      <tr>
        ...
        <td>
          <select name="st<?php echo $lignes; ?>" class="w3-select" id="st" >
...
<?php   
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
      $_sel = $bdd->prepare("SELECT distinct ida,ljr,etu,cour,statut FROM absence,etudiant,seance WHERE absence.idetud=etudiant.idp ");
    $_sel->execute();
    ?>

 <div class="table-responsive ">
  <h4 style="margin-left: 10px;">Mes Absences</h4>
   <table style="width:820px; margin-left:10px;">
    <thead>
     <tr>
       <th>Date</th>
...
?>  
     <tr>
       ...
       <td><?php echo $w->cour; ?></td>
       <td><?php echo $w->statut; ?></td>
       <td><a href="index_control_presence_action.php?idpo=<?php echo $w->ida;?>" target="_parent"  style="text-decoration:none;">Action</a></td>
       <?php
                    }
                    ?>
     </tr>
