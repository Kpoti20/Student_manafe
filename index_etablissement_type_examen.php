...
{
    $no = htmlspecialchars(trim($_POST['nom']));
    $de = htmlspecialchars(trim($_POST['des']));
        ...
        if($deslength <= 35)
       {
        if($de)
        {
           ...
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                  $inserttut = $bdd->prepare("INSERT INTO examen(noe,de) VALUES(?,?)");
                  $inserttut->execute(array($no,$de)); 
                  $no = "";
    			        $de = "";
                  "<br/>";
...
<?php 
      include_once 'index_etablissement_type_examen.php';
    ?>
<!-- Sidenav/menu -->
<nav class="w3-sidenav w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidenav"><br>
  ...
  <!--<a href="#contact" onclick="w3_close()" class="w3-padding"><i class="fa fa-envelope fa-fw w3-margin-right"></i>CONTACT</a>-->
   ...
   <a href="index_etablissement_type_examen.php?id=<?php echo ($_SESSION['id']); ?>" onclick="w3_close()" class="w3-padding"><i class="fa  fa fa-check-square-o fa-fw w3-margin-right"></i>EXAMEN</a>
...
<form class="w3-container" method="POST" action="" >
  <p>
              ...
              <input class="w3-input "  autocomplete="off" type="text" value="<?php if (isset($nom)) { echo $nom; } ?>" placeholder="Le nom de l'examen" id="nom" name="nom"></p>
              <p>
              <label for="des" >Description</label>
              <input class="w3-input" type="text"  value="<?php if (isset($des)) { echo $des; } ?>" autocomplete="off" placeholder="La description de l'examen" id="des" name="des"  ></p>
  ...
  <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Liste des Examens</i></h2>
  <?php   
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
    
    
      $_select = $bdd->prepare("SELECT * FROM examen order by ide desc");
...
?>    
      <tr>
        <td><?php echo $s->ide;?></td>
        <td><?php echo $s->noe;?></td>
        <td><?php echo $s->de;?></td>
        
        <td>
       <a href="index_etablissement_type_examen_action.php?ide=<?php echo $s->ide;?>" target="_parent"  style="text-decoration:none;">Action</a></td>
		     <?php
                    }
                    ?>
