        ...
        if($deslength <= 35)
       {
        if(preg_match("/^[a-zA-Z ]*$/",$des))
        {
           ...
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                  $inserttut = $bdd->prepare("INSERT INTO filiere(NomFil,Desc_f) VALUES(?,?)");
...
<?php 
      include_once 'index_etablissement_filiere.php';
    ?>
<!-- Sidenav/menu -->
...
<form class="w3-container" method="POST" action="" >
  <p>
              ...
              <input class="w3-input "  autocomplete="off" type="text" value="<?php if (isset($nom)) { echo $nom; } ?>" placeholder="Le nom de la filiere" id="nom" name="nom"></p>
              <p>
              <label for="des" >Description</label>
              <input class="w3-input" type="text"  value="<?php if (isset($des)) { echo $des; } ?>" autocomplete="off" placeholder="La description de la filiere" id="des" name="des"  ></p>
  ...
  <?php   
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
      ...
      $_select = $bdd->prepare("SELECT * FROM filiere order by Codefil desc");
...
?>    
      <tr>
        <td><?php echo $s->CodeFil;?></td>
        <td><?php echo $s->NomFil;?></td>
        <td><?php echo $s->Desc_f;?></td>
        
        <td>
       <a href="index_etablissement_filiere_action.php?CodeFil=<?php echo $s->CodeFil;?>" target="_parent"  style="text-decoration:none;">Action</a></td>
		     <?php
                    }
                    ?>
