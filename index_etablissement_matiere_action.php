<?php
    ...
    if(!empty($no) AND !empty($de))
  {
        ...
        if($deslength <= 35)
       {
        if(preg_match("/^[a-zA-Z ]*$/",$de))
        {
           ...
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                        $_update = $bdd->prepare("UPDATE matiere SET nom='$no', libelle='$de' WHERE idm=$Codemat ");
                        $_update->execute();
    ...
    {
      $erreur="Le nom ne dois pas depasser 20 caracteres!!!";
    }
  }
  else
  {
    $erreur = "veuillez remplir tous les champs!";
          ...
          if(isset($_POST['supprimer']))
           {
           $Codemat=$_POST['mat'];
           $_delete = $bdd->prepare("DELETE  FROM matiere WHERE idm=$Codemat");
           $_delete->execute();
...
<?php 
      include_once 'index_etablissement_matiere_action.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <?php   
            $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
            $Codemat=$_GET['idm'];
            $_select = $bdd->prepare("SELECT * FROM matiere WHERE idm=$Codemat ");
            $_select->execute();
            while ($s=$_select->fetch(PDO::FETCH_OBJ)) {
          ?>  
		<div class="w3-container w3-grey">
  <h2>Modification de la matiere : <?php echo $s->nom; ?></h2>
</div>
<br/>
<form class="w3-container" method="POST" action="">
   <p>
          <input type="hidden"  value="<?php echo $s->idm; ?>" readonly="true" name="mat">
          <label>Nom</label>
          <input class="w3-input" value="<?php echo $s->nom;?>" autocomplete="off" name="nom" type="text"></p>
          <p>
          <label>Description</label>
          <input class="w3-input" value="<?php echo $s->libelle; ?>" autocomplete="off" name="desc" type="text"></p>
  <p><button  type="submit" name="valider" class="w3-btn w3-teal w3-round-xxlarge">Sauvegarder</button>
...
<?php 
   ...
   <?php   
        ...
        <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Suppression de la matiere : <?php echo $z->nom;?> </i></h2>
    <p style="margin-left:15px;">
      A moins que vous ne trouviez plus l'importance de cette matiere dans votre liste des matieres, vous avez la possibilité de <a href="#demo" style="text-decoration:none; cursor:pointer;" data-toggle="collapse">supprimer la matiere en question</a>
    </p>
    <div id="demo" style="margin-left:15px;" class="collapse">
    Voulez-vous réellement supprimer cette matiere : <?php echo $z->nom; ?> ?
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->idm; ?>" readonly="true" name="mat">
 ...
 ?>
  ...
  <?php
      ...
      <?php   
        ...
        ?>
  <p class="w3-medium" style="margin-left:800px;"><b><i class="fa fa fa-arrow-left fa-fw w3-margin-right w3-text-teal"></i><a href="index_etablissement_matiere.php?id=<?php echo ($_SESSION['id']);?>" target="_parent" class="w3-padding-2" style="text-decoration:none;">Retour Matiere</a></b></p>
