    ...
    if($nomlength <= 10)
    {
      if (preg_match("/^[a-zA-Z ]*$/",$no)) 
      {
        if($deslength <= 35)
       {
        ...
        {
           $reqenre=$bdd->prepare("SELECT * FROM filiere WHERE NomFil = ? and Desc_f = ? ");
           $reqenre->execute(array($no,$de));
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                        $_update = $bdd->prepare("UPDATE filiere SET NomFil='$no', Desc_f='$de' WHERE CodeFil=$CodeFil ");
                        $_update->execute();
  ...
  else
  {
    $erreur = "veuillez remplir tous les champs!";
          ...
          if(isset($_POST['supprimer']))
           {
           $CodeFil=$_POST['fil'];
           $_delete = $bdd->prepare("DELETE  FROM filiere WHERE CodeFil=$CodeFil");
           $_delete->execute();
...
<?php 
      include_once 'index_etablissement_filiere_action.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-panel w3-card-12 w3-margin-bottom w3-white" style="width:1050px;">
   <?php   
            ...
            $_select = $bdd->prepare("SELECT * FROM filiere WHERE CodeFil=$CodeFil ");
            $_select->execute();
            while ($s=$_select->fetch(PDO::FETCH_OBJ)) {
          ?>  
		<div class="w3-container w3-grey">
  <h2>Modification de la filiere :  <?php echo $s->NomFil;?></h2>
</div>
<br/>
 <form method="POST" action="">
    
             <p>
          <input type="hidden"  value="<?php echo $s->CodeFil; ?>" readonly="true" name="fil">
          <label>Nom</label>
          <input class="w3-input" value="<?php echo $s->NomFil;?>" autocomplete="off" name="nom" type="text"></p>
          <p>
          <label>Description</label>
          <input class="w3-input" value="<?php echo $s->Desc_f;?>" autocomplete="off" name="desc" type="text"></p>
...
<?php 
   ...
   <?php   
    ...
    Voulez-vous réellement supprimer cette filiere : <?php echo $z->NomFil; ?> ?
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->CodeFil; ?>" readonly="true" name="fil">
 ...
 ?>
  ...
  <?php
      ...
      <?php   
        ...
        ?>
  <p class="w3-medium" style="margin-left:800px;"><b><i class="fa fa fa-arrow-left fa-fw w3-margin-right w3-text-teal"></i><a href="index_etablissement_filiere.php?id=<?php echo ($_SESSION['id']);?>" target="_parent" class="w3-padding-2" style="text-decoration:none;">Retour Filiere</a></b></p>
