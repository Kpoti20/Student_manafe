    ...
    if(!empty($no) AND !empty($de))
  {
    ...
    if($nomlength <= 10)
    {
        ...
        if($deslength <= 35)
       {
           ...
           $reqenre=$bdd->prepare("SELECT * FROM examen WHERE noe = ? and de = ? ");
           $reqenre->execute(array($no,$de));
           $enregistrexist =$reqenre->rowcount();
              if($enregistrexist == 0)
              { 
                        $_update = $bdd->prepare("UPDATE examen SET noe='$no', de ='$de' WHERE ide=$ide ");
                        $_update->execute();
  ...
  else
  {
    $erreur = "veuillez remplir tous les champs!";
          ...
          if(isset($_POST['supprimer']))
           {
           $ide=$_POST['exa'];
           $_delete = $bdd->prepare("DELETE  FROM examen WHERE ide=$ide");
           $_delete->execute();
...
<?php 
      include_once 'index_etablissement_type_examen_action.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-panel w3-card-12 w3-margin-bottom w3-white" style="width:1050px;">
   <?php   
            $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
            $ide=$_GET['ide'];
            $_select = $bdd->prepare("SELECT * FROM examen WHERE ide=$ide ");
            $_select->execute();
            while ($s=$_select->fetch(PDO::FETCH_OBJ)) {
          ?>  
		<div class="w3-container w3-grey">
  <h2>Modification de l'examen' :  <?php echo $s->noe;?></h2>
</div>
<br/>
 <form method="POST" action="">
    
             <p>
          <input type="hidden"  value="<?php echo $s->ide; ?>" readonly="true" name="exa">
          <label>Nom</label>
          <input class="w3-input" value="<?php echo $s->noe;?>" autocomplete="off" name="nom" type="text"></p>
          <p>
          <label>Description</label>
          <input class="w3-input" value="<?php echo $s->de;?>" autocomplete="off" name="desc" type="text"></p>
...
<?php 
   ...
   <?php   
        ...
        <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Suppression de l'examen du : <?php echo $z->noe;?> </i></h2>
    <p style="margin-left:15px;">
      A moins que vous ne trouviez plus l'importance de cet examen dans la liste des examens, vous avez la possibilité de <a href="#demo" style="text-decoration:none; cursor:pointer;" data-toggle="collapse">supprimer l'examen en question</a>
    </p>
    <div id="demo" style="margin-left:15px;" class="collapse">
    Voulez-vous réellement supprimer cet examen : <?php echo $z->noe; ?> ?
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->ide; ?>" readonly="true" name="exa">
 ...
 ?>
  ...
  <?php
      ...
      <?php   
        ...
        ?>
  <p class="w3-medium" style="margin-left:800px;"><b><i class="fa fa fa-arrow-left fa-fw w3-margin-right w3-text-teal"></i><a href="index_etablissement_type_examen.php?id=<?php echo ($_SESSION['id']);?>" target="_parent" class="w3-padding-2" style="text-decoration:none;">Retour Examen</a></b></p>
