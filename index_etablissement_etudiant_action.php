...
<?php
	include_once 'index_etablissement_etudiant_action.php';
?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <?php   
            $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
            $Codet=$_GET['idp'];
            $_select = $bdd->prepare("SELECT * FROM etudiant WHERE idp=$Codet ");
            $_select->execute();
            while ($s=$_select->fetch(PDO::FETCH_OBJ)) {
          ?>  
		<div class="w3-container w3-grey">
  <h2>Modification de l'étudiant(e) :  <?php echo $s->nom_e;?></h2>
</div>
<br/>
	<form method="POST" action="">
    
             <p>
          <input type="hidden"  value="<?php echo $s->idp; ?>" readonly="true" name="et">
          <label>Nom</label>
          <input class="w3-input" value="<?php echo $s->nom_e;?>" autocomplete="off" name="nom" type="text"></p>
          <p>
          <label>Prenom 1</label>
          <input class="w3-input" value="<?php echo $s->pren;?>" autocomplete="off" name="pre" type="text"></p>
          <p>
          <label>Prenom 2</label>
          <input class="w3-input" value="<?php echo $s->preno;?>" autocomplete="off" name="pre1" type="text"></p>
          <p>
          <label>Sexe</label>
          <select name="sx" class="w3-select"  id="sx" >
              <option><?php echo $s->sexe;?></option>
          ...
          <p>
          <label>Contact</label>
          <input class="w3-input" value="<?php echo $s->contact_e;?>" autocomplete="off" name="ct" type="tel"></p>
			 <p>
          <label>Filiere</label>
          
          <select name="fil" class="w3-select"  id="fil" >
                    <option><?php echo $s->code_f;?></option>
              <?php
                   ...
                   {
              ?>
                    <option><?php echo $p->NomFil ; ?></option>
                   <?php
                   }
                   ?>
		  ...
		  <button type="reset" class="w3-btn w3-teal w3-round-xxlarge">Annuler</button>
		  </p>
	</form><br/>
<?php 
      }
...
<?php   
        ...
        <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Suppression de l'étudiant : <?php echo $z->nom_e;?> </i></h2>
    ...
    Voulez-vous réellement supprimer l'étudiant : <?php echo $z->nom_e; ?>
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->idp; ?>" readonly="true" name="et">
  ...
  <p class="w3-medium" style="margin-left:800px;"><b><i class="fa fa fa-arrow-left fa-fw w3-margin-right w3-text-teal"></i><a href="index_etablissement_etudiant.php?id=<?php echo ($_SESSION['id']);?>" target="_parent" class="w3-padding-2" style="text-decoration:none;">Retour Etudiant</a></b></p>
    </div>    
