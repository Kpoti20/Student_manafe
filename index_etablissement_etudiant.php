                ...
                if($sx=="Masculin" or $sx=="Féminin")
               {
                  if($ctlength>=2)
                 {
                    if($fil!="Selectionnez la filiere de l'étudiant")
                   {
                        ...
                        if($enregistrexist == 0)
                        { 
                            $inserttut = $bdd->prepare("INSERT INTO etudiant(nom_e,pren,preno,sexe,contact_e,code_f) VALUES(?,?,?,?,?,?)");
                            $inserttut->execute(array($nom,$pre,$pre1,$sx,$ct,$fil)); 
                           $update = $bdd->prepare("UPDATE etudiant SET etu=(SELECT concat(nom_e,' ',pren,' ',preno)) WHERE idp=(SELECT MAX(idp))");
                            $update->execute();
                            $nom="";
                            $pre="";
                            $pre1="";
...
<?php 
      include_once 'index_etablissement_etudiant.php';
    ?>
<!-- Sidenav/menu -->
...
<div class="w3-main" style="margin-left:300px">
  ...
  <h2>Enregistrement Etudiant(e)s</h2>
</div>
<br/>
<form class="w3-container" method="POST" action="">
  <p>
  <label for="nom">Nom</label>
  <input class="w3-input" type="text" autocomplete="off" type="text" value="<?php if (isset($nom)) { echo $nom; } ?>" placeholder="Le nom de l'étudiant" id="nom" name="nom"></p>
 <p>
              ...
              <input class="w3-input" type="tel" autocomplete="off" value="<?php if (isset($ct)) {echo $ct; } ?>" placeholder="Contact de l'étudiant" id="ct" name="ct"></p>
			  <p>
  ...
  <?php
      ...
      <?php   
		...
		<h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Liste des Etudiant(e)s</i></h2>
<?php   
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolarite1','root', '');
    
    
      $_select = $bdd->prepare("SELECT * FROM etudiant order by idp desc limit 10");
...
?>    
      <tr>
        ...
        <td><?php echo $s->nom_e;?></td>
        <td><?php echo $s->pren;?></td>
        <td><?php echo $s->preno;?></td>
        <td><?php echo $s->sexe;?></td>
		<td><?php echo $s->contact_e;?></td>
        <td><?php echo $s->code_f;?></td>
        <td>
       <a href="index_etablissement_etudiant_action.php?idp=<?php echo $s->idp;?>" target="_parent"  style="text-decoration:none;">Action</a></td>
		     <?php
                    }
                    ?>
